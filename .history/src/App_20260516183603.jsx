...
const enPromo      = (p) => !PROMO_EXCLUS.includes(p.nom);
...
- Si on te demande comment payer : Wave (${WAVE_DISPLAY}) ou Cash à la livraison
...
const ProductCard = ({ p, senteurIdx, setSenteurIdx, onAddToCart, onToggleFavori, favoris, cibleRef }) => {
  ...
  // Scroll automatique si ce produit est la cible ✅
  useEffect(() => {
...
export default function App() {
  ...
  const [paymentMode, setPaymentMode] = useState("wave"); // "wave" | "livraison"
  const [wavePaid, setWavePaid]       = useState(false);  // true après clic sur "Payer sur Wave"
  ...
  // Reset l'état "déjà payé sur Wave" si le panier ou le mode change
  useEffect(() => { setWavePaid(false); }, [cart, paymentMode]);
  ...
  // ── PAIEMENT WAVE ───────────────────────────────────────────────────────────
  const openWave = () => {
    // Wave Business : lien marchand avec montant pré-rempli ✅
  ...
  const sendOrder = () => {
    ...
    const paiementTxt = paymentMode === "wave"
      ? `💳 Paiement : Wave Business ✅ (paye en ligne)`
  ...
  // ── HANDLER DU BOUTON FINALISER (logique selon mode + état) ─────────────────
  const handleFinaliser = () => {
    if (paymentMode === "wave" && !wavePaid) { openWave(); return; }
    sendOrder();
  };
  ...
  const finaliserLabel  =
    paymentMode === "wave" && !wavePaid ? `💸 Payer ${cartTotal.toLocaleString("fr-FR")} FCFA sur Wave` :
    paymentMode === "wave" &&  wavePaid ? "✅ J'ai payé — Envoyer ma commande sur WhatsApp" :
                                          "💬 Finaliser sur WhatsApp";
  const finaliserBg     =
    paymentMode === "wave" && !wavePaid ? `linear-gradient(135deg,${WAVE_BLUE},#0ea5d4)` :
                                          `linear-gradient(135deg,${WA_GREEN},#1a9e4a)`;

  const produitCibleRef = useRef(null); // nom du produit sur lequel scroller

  // ── Navigation vers un produit cliqué dans le chat ────────────────────────
  const goToProduit = (label) => {
    ...
    // Positionner le carrousel sur la bonne senteur
  ...
  // ── Parser le texte : [[Produit - Senteur]] → chips cliquables ─────────────
  const parseMessage = (text, small) => {
    const parts = text.split(/\[\[(.+?)\]\]/g);
    return parts.map((part, i) => {
      if (i % 2 === 1) {
        // C'est un nom de produit cliquable
        return (
              ...
              <>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px", marginBottom: "14px" }}>
                  <button onClick={() => setPaymentMode("wave")}
                    style={{ padding: "14px 8px", borderRadius: "14px", border: `1.5px solid ${paymentMode === "wave" ? WAVE_BLUE : "rgba(255,255,255,0.1)"}`, background: paymentMode === "wave" ? `${WAVE_BLUE}1f` : "rgba(255,255,255,0.04)", cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: "4px" }}>
                ...
                {paymentMode === "wave" && (
                  <div style={{ background: `${WAVE_BLUE}10`, border: `1px solid ${WAVE_BLUE}44`, borderRadius: "14px", padding: "16px", marginBottom: "18px" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "12px" }}>
                      ...
                      <span style={{ color: WAVE_BLUE, fontSize: "14px", fontWeight: "800" }}>Paiement Wave</span>
                      {wavePaid && <span style={{ marginLeft: "auto", background: "#27ae60", color: "#fff", fontSize: "10px", fontWeight: "800", padding: "3px 10px", borderRadius: "20px" }}>✅ Wave ouvert</span>}
                    </div>
                    <div style={{ color: "rgba(255,255,255,0.75)", fontSize: "13px", lineHeight: "1.6", marginBottom: "12px" }}>
                      Envoyez <strong style={{ color: WAVE_BLUE }}>{cartTotal.toLocaleString("fr-FR")} FCFA</strong> au numéro Wave :
                    </div>
                    <div onClick={() => { try { navigator.clipboard?.writeText(WAVE_DISPLAY); showToast("📋 Numéro copié !"); } catch {} }}
                      style={{ background: "rgba(0,0,0,0.4)", borderRadius: "10px", padding: "12px 14px", display: "flex", justifyContent: "space-between", alignItems: "center", cursor: "pointer", border: `1px dashed ${WAVE_BLUE}66`, marginBottom: "8px" }}>
                      <div style={{ color: "#fff", fontSize: "17px", fontWeight: "800", letterSpacing: "0.5px" }}>{WAVE_DISPLAY}</div>
                      <div style={{ color: WAVE_BLUE, fontSize: "11px", fontWeight: "700" }}>📋 Copier</div>
                    </div>
                    <div style={{ color: "rgba(255,255,255,0.45)", fontSize: "11px", lineHeight: "1.5" }}>
                      💡 <strong>1.</strong> Cliquez sur "Payer sur Wave" ci-dessous • <strong>2.</strong> Effectuez le transfert dans l'app Wave • <strong>3.</strong> Revenez ici et confirmez votre commande sur WhatsApp
                    </div>
                  </div>
                )}
                ...
                <button className="tbtn" onClick={handleFinaliser} disabled={!canFinaliser}
                  style={{ width: "100%", padding: "16px", borderRadius: "16px", background: canFinaliser ? finaliserBg : "rgba(255,255,255,0.1)", color: canFinaliser ? "#fff" : "rgba(255,255,255,0.3)", fontSize: "15px", fontWeight: "800", border: "none", cursor: canFinaliser ? "pointer" : "not-allowed", animation: paymentMode === "wave" && !wavePaid && canFinaliser ? "pulseBlue 1.8s ease-in-out infinite" : "none" }}>
                  {finaliserLabel}
                </button>
                {paymentMode === "wave" && wavePaid && (
                  <button className="tbtn" onClick={() => setWavePaid(false)}
        ...
        <div style={{ width: "100%", maxWidth: "480px", padding: "20px", flex: 1 }}>
          <div style={{ background: "rgba(255,255,255,0.04)", borderRadius: "20px", border: `1px solid ${GOLD}22`, padding: "24px", display: "flex", flexDirection: "column", gap: "16px" }}>
            ...
            {[
              ...
              { icon: "💸", label: "Wave",      value: WAVE_DISPLAY },
            ].map((info, i) => (
