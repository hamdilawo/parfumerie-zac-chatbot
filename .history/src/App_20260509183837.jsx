...
export default function App() {
  ...
  const goToProduit = (label) => {
    // label = "Collection Privée - Bakara" ou "Phantom Paco Rabanne"
    const [nomProduit, nomSenteur] = label.split(" - ").map(s => s.trim());
    const produit = PRODUITS.find(p => p.nom === nomProduit);
    if (!produit) return;

    // Si senteur précisée, on positionne le carrousel
    ...
    // Filtrer la bonne catégorie et naviguer
    setCatFilter(produit.cat);
    setSearch(nomProduit);
    setActivePage("catalogue");
    setChatOverlay(false);

    // Effacer la recherche après 100ms pour laisser l'utilisateur voir le produit
