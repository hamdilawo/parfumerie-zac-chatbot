...
const ChatMessage = ({ msg, gold, pColor }) => (
  <div className="msg" style={{ display: "flex", justifyContent: msg.role === "user" ? "flex-end" : "flex-start", alignItems: "flex-end", gap: "8px" }}>
    ...
    <div style={{
      ...
      background: msg.role === "user" ? `linear-gradient(135deg, ${pColor}, ${pColor}bb)` : "rgba(255,255,255,0.07)",
...
export default function App() {
  ...
  const isFavori = (nom) => favoris.includes(nom);

  // ── Catalogue filtré (useMemo) ────────────────────────────────────────────
  ...
  const send = useCallback(async (text) => {
    ...
    // Fix : si on envoie "catalogue" depuis l'overlay, on navigue et on ferme ✅
    if (msg.includes("catalogue") || msg.includes("Voir le catalogue")) {
  ...
  const sendOrder = () => {
    ...
    const lignes = cart.map(item => {
      ...
      const promoTag = p && enPromo(p) ? ` (Promo Tabaski -${Math.round(PROMO_TAUX * 100)}%25)` : "";
      const label = item.senteur ? `${item.nom} - ${item.senteur}` : item.nom;
      const vol   = item.volume  ? ` ${item.volume}ml` : "";
      return `• ${item.qty}× ${label}${vol} — ${(pu * item.qty).toLocaleString("fr-FR")} FCFA${promoTag}`;
    }).join("%0A");
  ...
  // ── Carrousel swipe ────────────────────────────────────────────────────────
  const handleTouchStart = (nom, e) => { touchStart.current[nom] = e.touches[0].clientX; };
  const handleTouchEnd   = (nom, senteurs, e) => {
    const diff = (touchStart.current[nom] ?? 0) - e.changedTouches[0].clientX;
    if (Math.abs(diff) < 40) return;
    setSenteurIdx(prev => {
      const cur = prev[nom] || 0;
      const next = diff > 0 ? (cur + 1) % senteurs.length : (cur === 0 ? senteurs.length - 1 : cur - 1);
      return { ...prev, [nom]: next };
    });
  };

  // ── Carte produit (extrait pour éviter la répétition) ──────────────────────
    ...
    return (
      <div className="pcard" style={{ background: "#161616", border: `1px solid ${GOLD}18`, borderRadius: "16px", overflow: "hidden", display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ position: "relative", height: "160px", overflow: "hidden", background: "#0d0d0d" }}
          onTouchStart={senteurs?.length > 1 ? e => handleTouchStart(p.nom, e) : undefined}
          onTouchEnd={senteurs?.length > 1   ? e => handleTouchEnd(p.nom, senteurs, e) : undefined}>
          ...
          {/* Bouton favori ❤️ */}
          <button onClick={() => toggleFavori(p.nom)} aria-label="Ajouter aux favoris"
        ...
        <div style={{ padding: "12px 14px", display: "flex", flexDirection: "column", flex: 1 }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              {senteurActive && (
                <div style={{ color: GOLD, fontWeight: "600", fontSize: "12px", marginTop: "3px" }}>
                  ✨ {senteurActive.nom}
                  {p.volume && <span style={{ color: "rgba(255,255,255,0.4)", fontWeight: "500", marginLeft: "5px" }}>{p.volume}ml</span>}
                </div>
              )}
          ...
          <button className="tbtn" onClick={() => addToCart(p, senteurActive)} disabled={stockAffiche === 0}
  ...
  }, [senteurIdx, favoris, addToCart, toggleFavori]);

  // ── Rendu input chat (partagé) ─────────────────────────────────────────────
  const ChatInput = ({ small }) => (
    <div style={{ display: "flex", gap: "10px", alignItems: "center" }}>
      <div style={{ flex: 1, display: "flex", alignItems: "center", background: "rgba(255,255,255,0.07)", borderRadius: "24px", border: "1px solid rgba(255,255,255,0.1)", padding: "0 16px", gap: "8px" }}>
        ...
        <input value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === "Enter" && send()}
          ...
          style={{ flex: 1, padding: small ? "11px 0" : "12px 0", background: "transparent", border: "none", color: "#fff", fontSize: "16px" /* 16px évite le zoom iOS ✅ */, fontFamily: "inherit" }} />
      </div>
      <button className="tbtn" onClick={() => send()} disabled={loading}
  ...
  return (
    <div style={{ minHeight: "100vh", background: BG, display: "flex", flexDirection: "column", alignItems: "center", fontFamily: "'SF Pro Display',-apple-system,sans-serif" }}>
      <style>{`
        ...
        .pcard{transition:all 0.25s;cursor:default}
      ...
      {activePage === "chat" && (
        <>
          <div style={{ width: "100%", maxWidth: "480px", flex: 1, padding: "16px 20px", display: "flex", flexDirection: "column", gap: "14px", overflowY: "auto", minHeight: "350px", maxHeight: "45vh" }}>
            {messages.map((msg, i) => <ChatMessage key={i} msg={msg} gold={GOLD} pColor={WA_GREEN} />)}
            {loading && (
          ...
          <div style={{ width: "100%", maxWidth: "480px", padding: "0 20px 10px", display: "flex", gap: "6px", flexWrap: "wrap" }}>
            {["Voir le catalogue 📦", "Meilleure vente ⭐", "Offrir un cadeau 🎁", "Commander 🛍️"].map((q, i) => (
              <button key={i} className="tbtn" onClick={() => send(q)} style={{ padding: "7px 12px", borderRadius: "20px", border: `1px solid ${GOLD}30`, background: `rgba(201,168,76,0.08)`, color: "rgba(255,255,255,0.7)", fontSize: "13px" }}>{q}</button>
            ))}
          </div>
      ...
      {activePage === "catalogue" && (
        <div style={{ width: "100%", maxWidth: "480px", flex: 1, padding: "16px 16px 30px", overflowY: "auto" }}>
          ...
          <div style={{ position: "relative", marginBottom: "12px" }}>
            ...
            {search && (
              <button onClick={() => setSearch("")} style={{ position: "absolute", right: "12px", top: "50%", transform: "translateY(-50%)", background: "none", border: "none", color: "rgba(255,255,255,0.4)", fontSize: "18px", cursor: "pointer" }}>✕</button>
            )}
          </div>
      ...
      {activePage === "favoris" && (
        <div style={{ width: "100%", maxWidth: "480px", flex: 1, padding: "16px 16px 30px", overflowY: "auto" }}>
          ...
          {produitsFavoris.length === 0 ? (
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
              {produitsFavoris.map((p, i) => <ProductCard key={i} p={p} />)}
            </div>
          )}
        </div>
      )}
                ...
                {[
                  ...
                  <div key={f.key} style={{ marginBottom: "14px" }}>
                    ...
                    <input value={order[f.key]} onChange={e => setOrder({ ...order, [f.key]: e.target.value })} placeholder={f.ph}
                      style={{ width: "100%", padding: "12px 16px", borderRadius: "12px", border: `1px solid ${GOLD}25`, background: "rgba(255,255,255,0.06)", color: "#fff", fontSize: "16px" /* iOS no-zoom ✅ */, fontFamily: "inherit", boxSizing: "border-box" }} />
                  </div>
                ))}
        ...
        <div style={{ width: "100%", maxWidth: "480px", padding: "20px", flex: 1 }}>
          <div style={{ background: "rgba(255,255,255,0.04)", borderRadius: "20px", border: `1px solid ${GOLD}22`, padding: "24px", display: "flex", flexDirection: "column", gap: "16px" }}>
            ...
            <button className="tbtn" onClick={() => window.open(`https://wa.me/${WHATSAPP}`, "_blank")}
              style={{ width: "100%", padding: "14px 8px", borderRadius: "16px", background: `linear-gradient(135deg,${WA_GREEN},#1a9e4a)`, color: "#fff", fontSize: "15px", fontWeight: "700", display: "flex", alignItems: "center", justifyContent: "center", gap: "8px", boxShadow: "0 4px 20px #25D36644" }}>
      ...
      <div style={{ width: "100%", maxWidth: "480px", textAlign: "center", padding: "10px", color: "rgba(255,255,255,0.15)", fontSize: "11px", letterSpacing: "2px", textTransform: "uppercase", borderTop: "1px solid rgba(255,255,255,0.05)" }}>
        Parfumerie de la Zac • Tous droits réservés 2025 {/* ✅ mis à jour */}
      </div>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", zIndex: 1000, display: "flex", alignItems: "flex-end", justifyContent: "center", animation: "fadeUp 0.25s ease forwards" }}>
          <div style={{ width: "100%", maxWidth: "480px", height: "85vh", background: BG, borderRadius: "20px 20px 0 0", border: `1px solid ${GOLD}33`, display: "flex", flexDirection: "column", boxShadow: `0 -10px 40px ${GOLD}33` }}>
            ...
            <div style={{ flex: 1, padding: "16px 18px", display: "flex", flexDirection: "column", gap: "12px", overflowY: "auto" }}>
              ...
              <div ref={overlayEndRef} /> {/* ✅ ref séparé */}
            </div>
            {/* Input overlay */}
