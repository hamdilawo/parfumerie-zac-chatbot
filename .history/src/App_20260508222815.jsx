...
const ProductCard = ({ p, senteurIdx, setSenteurIdx, onAddToCart, onToggleFavori, favoris }) => {
  ...
  return (
    <div style={{ background: "#161616", border: `1px solid ${GOLD}18`, borderRadius: "16px", overflow: "hidden", display: "flex", flexDirection: "column", transition: "all 0.25s" }}>
...
export default function App() {
  ...
  const sendOrder = () => {
    ...
    const txt = `Bonjour Matel ! 🖤%0AJe souhaite commander :%0A%0A👤 Nom : ${order.name}%0A📱 Tel : ${order.phone}%0A📍 Quartier : ${order.quartier}%0A%0A🌹 Mes articles :%0A${lignes}%0A%0A💰 Total : ${cartTotal.toLocaleString("fr-FR")} FCFA%0A%0AMerci ✨`;
    window.open(`https://wa.me/${WHATSAPP}?text=${txt}`, "_blank");
  };

  const canOrder = order.name && order.phone && order.quartier && cart.length > 0;

  // ── Rendu messages chat (inline pour éviter props drilling) ─────────────────
    ...
    <>
      {messages.map((msg, i) => (
        <div key={i} className="msg" style={{ display: "flex", justifyContent: msg.role === "user" ? "flex-end" : "flex-start", alignItems: "flex-end", gap: "8px" }}>
          ...
          <div style={{ maxWidth: "75%", padding: "11px 15px", borderRadius: msg.role === "user" ? "20px 20px 4px 20px" : "20px 20px 20px 4px", background: msg.role === "user" ? `linear-gradient(135deg,${WA_GREEN},${WA_GREEN}bb)` : "rgba(255,255,255,0.07)", color: msg.role === "user" ? "#fff" : "rgba(255,255,255,0.9)", fontSize: small ? "14px" : "15.5px", lineHeight: "1.6", border: msg.role === "assistant" ? "1px solid rgba(255,255,255,0.08)" : "none" }}>
            {msg.content}
          </div>
        </div>
      ))}
            ...
            {cart.length === 0 ? (
              ...
              <>
                ...
                <button className="tbtn" onClick={sendOrder} disabled={!canOrder}
                  style={{ width: "100%", padding: "16px", borderRadius: "16px", background: canOrder ? `linear-gradient(135deg,${WA_GREEN},#1a9e4a)` : "rgba(255,255,255,0.1)", color: canOrder ? "#fff" : "rgba(255,255,255,0.3)", fontSize: "16px", fontWeight: "700", border: "none", cursor: canOrder ? "pointer" : "not-allowed" }}>
